...
See the `turn_test` & 'msg_test' function for a usage example.
"""
...
from convokit import stopwords, has_agreement, has_disagreement
class Constructive:
    def __init__(self,path):
        ...
        with open(os.path.join(path, 'my_geo.txt')) as f:
            self.lexicons['geo'] = [line.strip().lower() for line in f]

        with open(os.path.join(path, 'my_meta.txt')) as f:
            self.lexicons['meta'] = [line.strip().lower() for line in f]

        with open(os.path.join(path, 'my_certain.txt')) as f:
            self.lexicons['certain'] = [line.strip().lower() for line in f]

        with open(os.path.join(path, 'my_hedges.txt')) as f:
            self.lexicons['hedge'] = [line.strip().lower() for line in f]


        self.lex_matcher = self.Lexicon(self.lexicons)
    class Lexicon(object):
        def __init__(self, wordlists):
            self.wordlists = wordlists
            self.regex = {cat: self.wordlist_to_re(wordlist) for cat, wordlist in wordlists.items()}
    ...
    def message_features(self, chat, reasons=None, stopwords=stopwords):
        ...
        for k, (user, tokens, tags) in enumerate(reasons):
            features = {}
            content_words = [w for w in self.get_content_tagged(tokens, tags)
            ...
            lex_counts = self.lex_matcher.count_words(tokens)
        ...
        for k, (user, tokens, tags, timestamp) in enumerate(chat):
            ...
            content_words = [w for w in self.get_content_tagged(tokens, tags) if
                             w not in stopwords]
    ...
    def msg_test(self):
        ...
        msg_feat, reason_feat =  self.message_features(test_chat, test_reasons)
    ...
    def turn_features(turns):
        ...
        for prev_turn, curr_turn in zip(turns, turns[1:]):
            ...
            pos_bigrams = Constructive.sublist_bigrams(pos)
            prev_pos_bigrams = set(Constructive.sublist_bigrams(prev_pos))
    ...
    def turn_test():
        ...
        for turn in Constructive.turn_features(Constructive.turn_from_chat(test_chat)):
            print(turn)
